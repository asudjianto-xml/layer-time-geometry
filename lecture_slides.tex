...
\begin{enumerate}
  \item \textbf{The Standard Picture} \hfill \textcolor{gray}{10 min}\\
    {\small Tokens, layers, hidden states}
  \item \textbf{Layers as Rotations} \hfill \textcolor{gray}{12 min}\\
    {\small Versors, bivectors, rotation planes}
  \item \textbf{When Order Matters} \hfill \textcolor{gray}{10 min}\\
    {\small Commutators, curvature, capacity}
  \item \textbf{The Decode Frontier} \hfill \textcolor{gray}{12 min}\\
    {\small Generation, hallucination signatures}
  \item \textbf{Seeing Inside \& Steering} \hfill \textcolor{gray}{10 min}\\
    {\small Dependency, intervention, the big picture}
...
\[
  H : (l, t) \;\longmapsto\; \mathbb{R}^{k}
\]
\vspace{-0.5em}
\begin{itemize}
  \item Vertical axis: \textbf{layers} (depth)
  \item Horizontal axis: \textbf{token positions} (context)
...
\begin{itemize}\small\setlength{\itemsep}{1pt}
  ...
  \item Required for valid GA in $\mathrm{Cl}(256, 0)$
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\includegraphics[width=\linewidth]{figures_ga_learning/ch04_covariance.pdf}
\vspace{-0.3em}
{\scriptsize\textbf{Figure:} Eigenvalue spectrum of the covariance. After whitening, retained dimensions have equal variance --- distances and angles become trustworthy.}
\end{column}
\end{columns}
\end{frame}
...
Each layer transition decomposes as:
\[
...
\vspace{0.5em}
\begin{center}
\alert{Key finding:} Rotation dominates stretch by \textbf{10--15$\times$} in norm.\\
...
\vspace{-0.5em}
{\small
\textbf{Reading the figure:} Each panel shows one prompt type. Blue bars = rotation (grade-2) norm; purple bars = stretch (grade-0) norm, per layer.
\textbf{Takeaway:} Blue towers over purple at every layer --- rotation outweighs stretch by \textbf{10--15$\times$}. Layers reshape \emph{which directions} matter, not how big they are.}
\end{frame}
...
\vspace{-0.3em}
{\scriptsize\textbf{Figure:} Each bar = $\|[\bivec{l}, \bivec{l+1}]\|_F$ for one layer pair. Tall bars = strong sequential coupling.}
\end{column}
\end{columns}
\end{frame}
...
\vspace{-0.8em}
{\small
\textbf{Reading the figure:} Each curve tracks the rotation angle at the newest generated token, plotted against decode step. Each line is a different layer.
\textbf{Takeaway:} Rotation planes are \textbf{not preserved} across steps --- they drift as context grows. Adversarial prompts show periodic cycling that mirrors stuck token loops.}
\end{frame}
...
\vspace{-0.5em}
{\small
\textbf{Reading the figure:} $x$-axis = decode step (tokens generated so far); $y$-axis = frontier capacity $C_{\text{acc}}$ at that step. Each curve is one prompt type.
\textbf{Takeaway:} Capacity decays as the model generates --- its sequential-reasoning budget \emph{drains}. Retrieval stays high and stable; adversarial declines into periodic cycling, signaling the model is running low on compositional resources.}
\end{frame}
...
\vspace{0.2em}
{\small\alert{Hallucination has a measurable geometric signature \emph{during} generation.}}
\end{column}
\end{columns}
\end{frame}
...
\vspace{0.2em}
{\small\alert{One GA object --- the Cayley bivector --- provides detection, diagnosis, and control.}}
\end{column}
\end{columns}
\end{frame}
...
\vspace{0.2em}
{\small\alert{Takeaway:} Multi-layer Cayley steering at moderate $\alpha$ overrides parametric resistance that context alone cannot --- a geometrically targeted nudge in the model's own rotation planes.}
\end{column}
\end{columns}
\end{frame}
...
\begin{enumerate}\small\setcounter{enumi}{4}
  ...
  \item Whether reasoning is causally aligned with context
\end{enumerate}
\end{column}
\end{columns}
...
\midrule
Versor decomp. & How much a layer rotates vs.\ stretches & $T = R \cdot P$ \\
Bivector field & Which directions each layer reshuffles & $B^{(l)} \in \mathfrak{so}(k)$ \\
Commutator & Does layer order matter? & $[B^{(l)}, B^{(l+1)}]$ \\
Capacity & Total sequential computation budget & $C_{\text{acc}} = \sum \|[\cdot]\|_F$ \\
Holonomy & Are layers and context entangled? & $R_{\text{loop}}$ \\
Dependency & Which layers the output cares about & $D_l = \|\partial s / \partial H^{(l)}\|_F$ \\
Frontier & How geometry evolves during generation & Bivectors at decode edge \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}\small\setlength{\itemsep}{1pt}
  \item Transformer layers are predominantly \textbf{rotations} in high-dimensional space
  \item \textbf{Non-commutativity} between layers is the geometric basis of sequential reasoning
  \item \textbf{Curvature} reveals where layer and token processing are entangled
  \item Hallucination produces a measurable \textbf{geometric signature} during generation
  \item Geometric understanding enables \textbf{plane-specific intervention}
\end{itemize}
\end{frame}
