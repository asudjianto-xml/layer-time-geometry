...
This appendix explains how decoder-only transformers are trained to generate
text, how they scale into ``large language models'' (LLMs), and why
understanding their internals matters.

% ──────────────────────────────────────────────────────────────────────────────
\section{The GPT idea: next-token prediction}

GPT (Generative Pre-trained Transformer) is based on a remarkably simple
objective: \textbf{predict the next token}.

Given a sequence of tokens $(x_1, x_2, \ldots, x_{t-1})$, the model
computes a probability distribution over the vocabulary for the next token:
\[
  P(x_t \mid x_1, \ldots, x_{t-1}) =
  \text{softmax}\!\bigl(\mathbf{W}_{\text{head}} \, \mathbf{h}^{(L)}_t\bigr),
\]
where $\mathbf{h}^{(L)}_t$ is the hidden state at the final layer~$L$ for
token position~$t$, and $\mathbf{W}_{\text{head}}$ is the \textbf{language
model head}---a linear projection from hidden dimension to vocabulary size.

\begin{keyidea}
Everything the model ``knows'' must be encoded in the hidden states
$\mathbf{h}^{(l)}_t$.  By studying the geometry of these hidden states, we
study how the model processes and stores information---which is exactly what
this book does.
\end{keyidea}

The training loss is the \textbf{cross-entropy} between the predicted
distribution and the actual next token:
\[
...
During training, the model processes the entire sequence at once but uses a
\textbf{causal mask} to prevent each token from attending to future tokens.
This means token~$t$ can only see tokens~$1, 2, \ldots, t$---exactly
mimicking the left-to-right generation process.

\subsection{Generation: autoregressive decoding}

To generate text, the model works one token at a time:
\begin{enumerate}[nosep]
\item Feed the prompt through the model.
\item Sample or select the most likely next token from $P(x_t \mid \ldots)$.
\item Append that token to the sequence.
\item Repeat from step~1 with the extended sequence.
\end{enumerate}

\begin{analogy}
A GPT model is like an author who writes one word at a time, each time
re-reading everything written so far to decide the next word.  The ``style''
and ``knowledge'' of the author are encoded in the weights; the
``thought process'' for each word is the forward pass through the layers.
\end{analogy}

% ──────────────────────────────────────────────────────────────────────────────
\section{Scaling: from GPT to large language models}

The GPT approach scales remarkably well.  Increasing model size (parameters),
...
\begin{tabular}{@{}lrrrl@{}}
\toprule
\textbf{Model} & \textbf{Parameters} & \textbf{Layers} & \textbf{Hidden dim} & \textbf{Year} \\
\midrule
GPT-2       & 1.5B  & 48  & 1600  & 2019 \\
GPT-3       & 175B  & 96  & 12288 & 2020 \\
LLaMA-2 7B  & 7B    & 32  & 4096  & 2023 \\
Qwen2.5-7B  & 7B    & 28  & 3584  & 2024 \\
Qwen2.5-32B & 32B   & 64  & 5120  & 2024 \\
LLaMA-3 70B & 70B   & 80  & 8192  & 2024 \\
\bottomrule
\end{tabular}
\end{center}
...
dimensions) and validate results on DeepSeek-R1-Distill-Qwen-7B and
Qwen2.5-32B.  The geometric framework applies to \emph{any} decoder-only
...
\begin{itemize}[nosep]
\item \textbf{Parameters:} more layers, wider hidden states, more attention
  heads.  A 7B-parameter model has $\sim$7 billion learned weights.
...
\item \textbf{Compute:} training GPT-3 required $\sim$3.6 $\times 10^{23}$
  floating-point operations.  Modern models require even more.
\end{itemize}

% ──────────────────────────────────────────────────────────────────────────────
\section{Key techniques in modern LLMs}

\subsection{Pre-training and fine-tuning}

LLM development typically has two stages:
\begin{enumerate}[nosep]
\item \textbf{Pre-training:} next-token prediction on a massive corpus.
  This gives the model broad language understanding and world knowledge.
\item \textbf{Fine-tuning:} additional training on curated data for
  specific tasks.  This includes \textbf{instruction tuning} (teaching
  the model to follow instructions) and \textbf{RLHF} (reinforcement
  learning from human feedback, aligning the model with human preferences).
\end{enumerate}

\subsection{In-context learning}

A remarkable property of LLMs is \textbf{in-context learning}: the ability
to perform new tasks by conditioning on examples provided in the prompt,
without updating any weights.

\begin{lstlisting}[caption={In-context learning: the model learns the pattern from examples}]
...
The model produces ``bonjour'' without any French-specific training.  How?
...
\subsection{The KV-cache and efficient inference}

During generation, the model caches the key and value matrices from previous
tokens (the \textbf{KV-cache}) so they don't need to be recomputed.  This
makes each new token require only one forward pass through the layers,
rather than reprocessing the entire sequence.
...
vectors produced at each layer and token position.  Here is how the pieces
connect:

\begin{center}\small
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Concept from this appendix} & \textbf{Where it appears in the book} \\
\midrule
Hidden states $\mathbf{h}^{(l)}_t$ & Chapter~1: the layer--time grid \\
Self-attention (token mixing) & Chapter~5: curvature from path non-commutativity \\
Feed-forward network (per-token) & Chapter~4: stretching in polar decomposition \\
Residual connections & Chapter~3: high layer-kernel similarity \\
Next-token prediction & Chapter~7: dependency via $\partial s / \partial H$ \\
Causal masking & Chapters~8--9: why last token is special \\
Scaling (layers, width) & Chapter~6: cross-model validation \\
\bottomrule
\end{tabular}
\end{center}

You do not need to understand every detail of the transformer to use the
geometric framework.  The key insight is simple: the hidden states exist,
they change from layer to layer, and we can measure \emph{how} they change
