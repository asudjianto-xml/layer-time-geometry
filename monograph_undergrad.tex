...
layer-time grid is such a vector (Chapter~\ref{chap:opening}).
...
$T^{(l)}$ (Chapter~\ref{chap:polar}), the best linear map from hidden
...
exact inverse does not exist (Chapter~\ref{chap:polar}).
...
\begin{itemize}[nosep]
  ...
  \item The stretch factor $P$ in the polar decomposition is PSD by
    construction (Chapter~\ref{chap:polar}).
\end{itemize}
...
information without changing magnitudes (Chapter~\ref{chap:polar}).
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Polar decomposition} (Chapter~\ref{chap:polar}): we factor
...
$z = e^{i\theta} \cdot r$ (Chapter~\ref{chap:polar}).
...
(Chapters \ref{chap:polar} and~\ref{chap:experiments}).
...
(Chapter~\ref{chap:polar}).  In code this is computed by
