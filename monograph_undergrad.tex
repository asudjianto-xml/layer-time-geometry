...
\begin{center}
  ...
  {\normalsize\color{DGray} H2O.ai}\\[2pt]
  {\normalsize\color{DGray} Center for Trustworthy AI Through Model Risk Management,}\\[1pt]
  {\normalsize\color{DGray} University of North Carolina Charlotte}\\[16pt]
...
\subsection*{Why this book is useful}

This book will not teach you to reverse-engineer every circuit in a
transformer. Methods like activation patching, probing, and circuit tracing
are powerful for explaining model behavior locally---identifying which heads,
neurons, or pathways drive a specific output. This book serves a different
purpose.

It teaches you how to \emph{measure the global computational regime} of a
model: where nontrivial computation concentrates across the full layer--time
field, how prompts and models differ internally, how dependency and control
are organized, and how to run practical diagnostics for reasoning-related
failures, memory use, and steering.

In that sense, the framework sits at a \textbf{mesoscopic layer} between
black-box evaluation (which tells you \emph{what} the model got wrong) and
fine-grained circuit tracing (which tells you \emph{which specific component}
is responsible):
...
\begin{tabular}{@{}l l l@{}}
\toprule
\textbf{Level} & \textbf{Method} & \textbf{Question answered} \\
\midrule
Black-box evaluation & Benchmarks, red-teaming &
  Did the model get it right? \\
\textbf{Layer--time geometry} & \textbf{This book} &
  \textbf{Where and how does computation organize?} \\
Circuit tracing & Activation patching, probing &
  Which head or neuron is responsible? \\
\bottomrule
\end{tabular}
\end{center}

Use layer--time geometry to find the interesting regions and regimes; use
tracing to zoom in mechanistically.

\subsection*{What this book is not claiming}

This book does not claim to have solved reasoning in the broad human sense,
and it does not try to reverse-engineer every transformer circuit. Its goal
is narrower and more practical: to measure how computation is organized
inside the model across layers and tokens. In this framework, geometry tells
you where interaction happens and how information is transformed; dependency
tells you which parts of that computation actually matter for the output.
Together, they provide operational diagnostics for reasoning-like
computation, memory use, and control.
...
\texttt{ch8\_reasoning\_memory\_control.ipynb} & Ch.~8: Reasoning-Like Computation, Memory, and Control \\
...
\item \textbf{Run controlled experiments} (Chapter~\ref{chap:experiments}).
  ...
  properties actually affect the geometry: task type? prompt structure?
  prompt length?
...
\begin{itemize}[nosep]
\item Extract and whiten hidden states from any Hugging Face transformer.
...
\end{itemize}
Compare the three summaries. How do the prompts differ in \emph{where}
curvature concentrates and \emph{how} dependency is distributed?
\begin{itemize}[nosep]
\item Does the factual prompt peak earlier or later than the reasoning prompt?
\item Does the reasoning prompt show more distributed dependency across layers?
\item Which differences look structural, and which might just reflect prompt form?
\end{itemize}
\end{tryit}
...
\begin{lstlisting}
# Hint: use the research API for custom whitening dimension
from layer_time import LayerTimeAnalyzer

analyzer = LayerTimeAnalyzer(model_name="Qwen/Qwen2.5-7B", device="cuda")

for k in [32, 64, 128, 256]:
    result = analyzer.analyse("The capital of France is",
                              whiten_components=k)
    curv = result.curvature_by_layer
    print(f"k={k:3d}: mean_curv={curv.mean():.4f}, "
          f"peak_layer={curv.argmax()}")
\end{lstlisting}
\end{tryit}
...
\begin{whycare}
Curvature tells you \emph{where} in the layer--time grid non-separable
interaction happens. Zero curvature means the local computation is close to
separable: moving across layers and tokens nearly commutes. High curvature
means the model is doing genuinely non-trivial interaction at that point.
Curvature is one of the most useful \emph{geometry} diagnostics in the
framework---but it is not the whole story. Later chapters add dependency
to distinguish interesting geometry from functionally important computation.
\end{whycare}
...
This is the most robust \emph{geometry result} in the framework: across
models, prompts, and tasks, curvature tends to concentrate in the final
layers.
\end{finding}
...
% ══════════════════════════════════════════════════════════════════════════════
\chapter{Reasoning-Like Computation, Memory, and Control}
...
\begin{pitfall}
\textbf{What this chapter is and is not doing.}
This chapter does not prove that transformers ``reason'' in the broad human
sense. It gives an \emph{operational internal definition} within this
framework. We call a computation \textbf{reasoning-like} when three things
happen together: non-separable interaction, non-degenerate propagation, and
distributed dependency across layers. This is a practical measurement
definition, not a final philosophical theory of reasoning.
\end{pitfall}

\section{An operational view of reasoning-like computation}

In this framework, we do not identify reasoning with one number such as
curvature. Instead, we look for a pattern. A prompt shows
\textbf{reasoning-like computation} when:

\begin{keyidea}
A transformer exhibits \textbf{reasoning-like computation} at layer $l$ and
token $t$ when three conditions hold simultaneously:
\begin{enumerate}[nosep]
\item \textbf{Interaction}: The curvature $\|\Omega(l,t)\|$ is
  significantly above baseline---the model is doing non-trivial,
  non-separable processing.
\item \textbf{Propagation}: The metric factor $P^{(l)}$ has high condition
  number---information continues to propagate without collapsing into a
  trivial channel.
\item \textbf{Dependency}: The dependency $D_{l,t}$ is high---the
  processing at this point actually matters for the output.
\end{enumerate}
In plain language: the model must be interacting, carrying information
forward, and using more than a trivial terminal decision layer. All three
must be present. Interaction without dependency is ``busy work.''
Dependency without interaction is just passing information through.
\end{keyidea}

\section{An operational view of memory}

Memory has at least two meanings. Information can be \emph{available} to the
model because it appears in the prompt or because it is stored in the
weights. But available information is not the same as \emph{used}
information. In this book, we focus on \textbf{functional memory}:
information that continues to influence the model's output across later
layers. Dependency lets us measure that usage directly.
...
\section{An operational view of control}

Control means changing the model's internal computation in a way that
changes the dependency profile and, ultimately, the output. In principle,
control can act through both rotation and stretching. In practice, our
experiments show that the dominant effective lever is the \textbf{metric
side}: modifying the eigenvalues of $P^{(l)}$ has much larger effects than
rotation-only interventions (Section~\ref{sec:metric-dominates} below).
The polar decomposition gives us two levers:
...
for l in range(result.n_layers - 1):
    ...
    if curv_active and stretch_active and dep_active:
        print(f"  Layer {l}: REASONING "
              ...
              f"dep={dep_norm[l]:.3f})")
\end{lstlisting}
Which layers contribute most to the framework's reasoning-like profile?
Are they concentrated at the end of the network?
\end{tryit}
...
Layer--time geometry does not replace circuit tracing. It answers a different
question. Tracing tells you which head, neuron, or pathway caused a
behavior. Layer--time geometry tells you what computational regime the model
is in, where non-trivial work concentrates, how prompts differ internally,
and how interventions reshape dependency. What this framework \emph{does not}
identify is which specific component is responsible for a particular output.

In practice, the two approaches fit together naturally: use layer--time
geometry to find the interesting regions and regimes; use tracing to zoom in
mechanistically.
...
\item \textbf{Experimental design:} Fang, Li, and Sudjianto,
  \emph{Design and Modeling for Computer Experiments} (Chapman \& Hall/CRC, 2006).
  \url{https://www.taylorfrancis.com/books/mono/10.1201/9781420034899}.
  Background on the DOE methodology used in Chapter~\ref{chap:experiments}.
\end{itemize}
